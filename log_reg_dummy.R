...
dtr <- xgb.DMatrix(data = data.matrix(tr_te_xgb[idx, ]), label = log1p(y2[idx]))

# eval dataset with idx == F
dval <- xgb.DMatrix(data = data.matrix(tr_te_xgb[!idx, ]), label = log1p(y2[!idx]))

# get full training dataset for testing the model
dtrain <- xgb.DMatrix(data = data.matrix(tr_te_xgb), label = log1p(y2))
...
pred_xgb_tr = as.data.frame(pred_xgb_tr)
pred_xgb_tr$y = y
mean_value = mean(y[y>0])
...
colnames(check) = "y"
check_falser = check %>% arrange(-y)
check_falser[round(nrow(check)*0.0129072,0),]
check$y[check$y < check_falser[round(nrow(check)*0.0129072,0),]] = 0
...
filer2 = merge(filer, check, by = "fullVisitorId", all.x = T)
